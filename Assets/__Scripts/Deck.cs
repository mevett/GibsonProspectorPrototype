...
public class Deck : MonoBehaviour
{
    ...
    public void ReadDeck(string deckXMLText)
    {
        ...
        // only for testing.  print(s);   // Terminal output to demonstrate we are reading XML file    

        // Read decorators for all Cards         
        decorators = new List<Decorator>(); // Init the List of Decorators         
        // Grab an PT_XMLHashList of all s in the XML file         
        ...
        for (int i = 0; i < xDecos.Count; i++)
        {
            ...
            // Copy the attributes of the  to the Decorator             
        ...
        // Grab an PT_XMLHashList of all the s in the XML file         
        ...
        for (int i = 0; i < xCardDefs.Count; i++)
        {
            ...
            // Grab an PT_XMLHashList of all the s on this              
            ...
            if (xPips != null)
            {
                for (int j = 0; j < xPips.Count; j++)
                {
                    ...
                    deco = new Decorator();
                    // <pip>s on the  are handled via the Decorator Class                     
                    deco.type = "pip";
            ...
            {
                cDef.face = xCardDefs[i].att("face");   // The base name for the 
                // card.  E.g., for the Jack, it'll be FaceCard_11.  Thus the
                // the Jack of Clubs is FaceCard_11C
            }
            cardDefs.Add(cDef);
        }
    }
